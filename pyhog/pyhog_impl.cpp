...
FeatureVector process(double* im, const int rows, const int cols, const int size, const int sbin ) {
    ...
    int blocks[] = {(int)round((double)dims[0]/(double)sbin), (int)round((double)dims[1]/(double)sbin)};

    double *hist = new double[blocks[0]*blocks[1]*18]();
    double *norm = new double[blocks[0]*blocks[1]]();

    // memory for HOG features
    int out[] = {max(blocks[0]-2, 0), max(blocks[1]-2, 0), 27+4};

    vector<double> feat(out[0]*out[1]*out[2]);

    int visible[] = {blocks[0]*sbin, blocks[1]*sbin};
...
#pragma omp parallel for  collapse(2)
    for (int x = 0; x < out[1]; x++) {
        for (int y = 0; y < out[0]; y++) {
            double *dst = feat.data() + x*out[0] + y;
